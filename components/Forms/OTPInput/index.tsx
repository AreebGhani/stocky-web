...
export function OTPInputComponent(props: OTPInputProps) {
    ...
    // Helper to set OTP values
    useEffect(() => {
        const customSetOtpValues = () => {
            if (changeOTPValues) {
                setOTPValues(changeOTPValues);
            }
        }
        customSetOtpValues();
    }, [changeOTPValues])
    ...
    const handleOnPaste = useCallback(
        (e: React.ClipboardEvent<HTMLInputElement>) => {
            ...
            setOTPValues(updatedOTPValues);
            setActiveInput(length - pastedData.length);
            const joinedOTP = updatedOTPValues.join('');
            handleOtpChange(updatedOTPValues);
        },
        [length, otpValues, getRightValue, handleOtpChange],
    );
